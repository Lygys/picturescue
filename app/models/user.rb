class User < ApplicationRecord
  ...
  def follow_requesting?(other_user)
    potential_followings.include?(other_user)
  end

  def follow_requested_by?(other_user)
    potential_followers.include?(other_user)
  end
  ...
  def follow_request(other_user)
    unless self == other_user
      follow_requests.find_or_create_by(follow_id: other_user.id)
    end
  end

  def remove_follow_request(other_user)
    follow_request = follow_requests.find_by(follow_id: other_user.id)
    follow_request.destroy if follow_request
  end

  def accept_follow_request(other_user)
    unless self == other_user
      Relationship.find_or_create_by(user_id: other_user.id, follow_id: id)
    end
  end

  def reject_follow_request(other_user)
    follow_request = FollowRequest.find_or_create_by(user_id: other_user.id, follow_id: id)
    follow_request.destroy if follow_request
  end

  def block(other_user)
    relationship = Relationship.find_by(user_id: other_user.id, follow_id: id)
    relationship.destroy if relationship
  end

  def remove(other_user)
    relationship = Relationship.find_by(user_id: id, follow_id: other_user.id)
    relationship.destroy if relationship
  end
